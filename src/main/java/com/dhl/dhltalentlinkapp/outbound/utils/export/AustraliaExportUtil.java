...
public class AustraliaExportUtil extends ExportDataUtil {

	private final static Logger LOGGER = LoggerManager.getLogger(AustraliaExportUtil.class);
	...
	public ExportData generateExportData(List<StructuredFormQADto> finalQuestAnswerList) {
		...
		finalQuestAnswerList.forEach(questAnswerDetails -> {
			...
			case "Suburb":
				if(StringUtils.isEmpty(exportData.getSuburb()))		
				exportData.setSuburb(questAnswerDetails.getAnswer());
				
				break;

			case "Applicable Enterprise Agreement":
				//exportData.setApplicable_enterprise_agreement(questAnswerDetails.getAnswer()); temporary change
				exportData.setApplicable_enterprise_agreement("");
				LOGGER.info("Applicable Enterprise Agreement, " + questAnswerDetails.getSource());
				break;
			...
			case "Tax File Number (TFN)":
				exportData.setNational_identifier(questAnswerDetails.getAnswer());
				LOGGER.info("Tax File Number (TFN), " + questAnswerDetails.getSource());
				break;

			/*
			 * 1. Address Type - default Home - are we going to send only home address or we
			 * should include mailing address also ? (addressed) 2. GID - ?? - from where to
			 * fetch candidate GID. (no updates) 3. Person ID: should it be
			 * TalentLink_{{candidate_application_id}} confirmed...(addressed) 4.
			 * DSC_Job_Code from DSC Global Job Name v2 - need to add logic 5.
			 * DSC_Location_Code - from Location Name v2 - need to add logic 7. Line Manager
			 * Full Name get gID from manager file and use it for Line Manager.- need to add
			 * logic , 6. Working_Hours and Working_Hours_Frequency from requisition form:
			 * should we take timeCount as work hours and workUnit as work_hours_frequency
			 * ?? <workTime> <timeCount>15.0</timeCount> <workPeriod>Day</workPeriod>
			 * <workUnit>Days</workUnit> </workTime> if Work Hours Frequency v2 present
			 * thats priority.. else take values from workTime timecount and workPeriod. 8.
			 * Salary Amount ?? should we take value from 'Salary &/or Other Benefits' from
			 * requisition form for Salary Amount ??" <permanentContract>
			 * <fixedSalary>4000</fixedSalary> 9. need to change
			 * 
		...
		try {
			...
			/*
			 * if (line_manager_full_name.get() != null) { Optional<Tlkuserdetails>
			 * tlkuserdetails = userManager.stream().filter(userDetails -> {
			 * 
			 * if (line_manager_full_name.get() .equals(userDetails.getFirstname().get() +
