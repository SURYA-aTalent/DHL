...
public class SingaporeExportUtil extends ExportDataUtil {

	private final static Logger LOGGER = LoggerManager.getLogger(SingaporeExportUtil.class);
	...
	public SingaporeExportData generateExportData(List<StructuredFormQADto> finalQuestAnswerList) {

		SingaporeExportData exportData = new SingaporeExportData();
		...
		AtomicReference<String> line_manager_full_name = new AtomicReference<String>();
		...
		finalQuestAnswerList.forEach(questAnswerDetails -> {
			...
			case "Phone (Mobile)":
				if (questAnswerDetails.getAnswer() != null && !StringUtils.isEmpty(questAnswerDetails.getAnswer())) {
					exportData.setContact_phone_number(questAnswerDetails.getAnswer());
			...
			case "Payroll":
				if (!StringUtils.isEmpty(questAnswerDetails.getAnswer())) {
					if (questAnswerDetails.getAnswer().equals(CommonConstants.SG_CO_MONTHLY_PAYROLL_VALUE)) {
						exportData.setPayroll(CommonConstants.SG_CO_MONTHLY_PAYROLL_NAME);
					} else if (questAnswerDetails.getAnswer().equals(CommonConstants.SG_RO_MONTHLY_PAYROLL_VALUE)) {
						exportData.setPayroll(CommonConstants.SG_RO_MONTHLY_PAYROLL_NAME);
					} else if (questAnswerDetails.getAnswer().equals(CommonConstants.SG_RO_Fortnightly_PAYROLL_VALUE)) {
						exportData.setPayroll(CommonConstants.SG_RO_Fortnightly_PAYROLL_NAME);
					}else
			...
			case "Legal Full Name":
				exportData.setLegal_name(questAnswerDetails.getAnswer());
				LOGGER.info("Legal Full Name, " + questAnswerDetails.getSource());
				break;

			case "Block or House Number":
				if (StringUtils.isEmpty(exportData.getBlock_or_house_number()))
				exportData.setBlock_or_house_number(questAnswerDetails.getAnswer());				
				break;

			case "Street Name":
				if (StringUtils.isEmpty(exportData.getStreet_name()))
				exportData.setStreet_name(questAnswerDetails.getAnswer());

				break;

			case "Level - Unit Number":
				if (StringUtils.isEmpty(exportData.getLevel_unit_number()))
				exportData.setLevel_unit_number(questAnswerDetails.getAnswer());				
				break;

			case "Building Name":
				if (StringUtils.isEmpty(exportData.getBuilding_name()))
				exportData.setBuilding_name(questAnswerDetails.getAnswer());		
				break;

			case "Local Grade v2":
				exportData.setLocal_grade(questAnswerDetails.getAnswer());
				LOGGER.info("Local Grade v2, " + questAnswerDetails.getSource());
				break;
			...
			case "NRIC Number":
				...
				LOGGER.info("NRIC Number, " + questAnswerDetails.getSource());
				break;
				
			case "Singapore Permanent Resident":
				if(!StringUtils.isEmpty(questAnswerDetails.getAnswer())) {
				exportData.setSingaporePermanentResident(questAnswerDetails.getAnswer());
				}
				LOGGER.info("Singapore Permanent Resident, " + questAnswerDetails.getSource());
				break;
				
			case "FIN":
				if(!StringUtils.isEmpty(questAnswerDetails.getAnswer())) {
				exportData.setFin(questAnswerDetails.getAnswer());
				}
				LOGGER.info("FIN, " + questAnswerDetails.getSource());
				break;

			case "Ethnicity":
				exportData.setEthnic_origin(removeLastComma(questAnswerDetails.getAnswer()));
				LOGGER.info("Ethnicity, " + questAnswerDetails.getSource());
				break;

			case "Religion":
				exportData.setReligion(removeLastComma(questAnswerDetails.getAnswer()));
				LOGGER.info("Religion, " + questAnswerDetails.getSource());
				break;
		...
		try {
			...
			/*
			 * LOGGER.info("line_manager_full_name " + line_manager_full_name.get()); if
			 * (line_manager_full_name.get() != null) { Optional<Tlkuserdetails>
			 * tlkuserdetails = userManager.stream().filter(userDetails -> { if
			 * (line_manager_full_name.get() .equals(userDetails.getFirstname().get() + " "
			 * + userDetails.getLastname().orElse(""))) return true; else return false;
			 * }).findFirst();
			 * 
			 ...
			 * line_manager_full_name.get()); } else {
			 * LOGGER.info("Line Manager Not Found"); }
			 */
